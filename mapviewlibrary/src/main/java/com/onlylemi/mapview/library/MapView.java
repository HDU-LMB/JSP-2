...
public class MapView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private List<MapBaseLayer> frontLayers;
            ...
            if (canvas != null) {
                ...
                if (isMapLoadFinish) {
                    for (MapBaseLayer layer : layers) {
                        if (layer.isVisible&&layer==mapLayer) {
                    ...
                    for (MapBaseLayer layer : frontLayers) {
                        layer.draw(canvas, currentMatrix, currentZoom, currentRotateDegrees);
                    }
                }
...
//                        Log.i(TAG, "rotate:" + currentRotateDegrees);
     ...
     * @return
     */
    public float[] convertScreenXY2MapXY(float x, float y) {
    ...
    public float[] convertMapXY2ScreenXY(float x, float y) {
        Matrix invertMatrix = new Matrix();
        float[] value = {x, y};
        currentMatrix.mapPoints(value);
        return value;
    }
    ...
    public boolean withFloorPlan(float x, float y) {
        float[] goal = convertScreenXY2MapXY(x, y);
    ...
    public void send2Front(MapBaseLayer layer) {
        MapBaseLayer lr= layer;
